...
(詳細は [slack-setup.md の手順 7](slack-setup.md#7-モデルの初回サブスクリプションアカウントで初回のみ))。

**Slack bot をリランク有効(デフォルト)でデプロイする場合**: 検索精度向上のため
Cohere Rerank 3.5 を使用する。Cohere は Marketplace 商品のため、回答生成の Claude Haiku 4.5 と
同様に初回 invoke 時に Marketplace サブスクリプションが自動確定される(Titan は Amazon 自社モデルで
サブスク対象外なので別扱い)。Cohere は Anthropic のようなユースケースフォーム提出は不要。
未確定のまま運用するとリランク実行時に `AccessDeniedException` が発生する。リランクが不要な場合は
`-c rerank=false` をデプロイ時に指定することで無効化できる(Cohere のモデルアクセス不要)。
